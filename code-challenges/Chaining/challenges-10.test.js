...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let finalArray = [];
  input.forEach(array => {
    finalI = array.filter(item => item % 5 === 0 && Number.isInteger(item))
    finalI = finalI.map(item => Math.pow(2, item))
    finalArray.push(finalI);
  })
  return finalArray;
...
let findShortest = (data) => {
  // Solution code here...
  let shortestHeight = data[0].height;
  let characterName;
  data.forEach(character => {
    if (parseInt(character.height) < shortestHeight) {
      shortestHeight = character.height;

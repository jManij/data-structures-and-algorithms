...
const findAnything = (arr, target) => {
  let finalArray = [];
  arr.forEach(str => {
    if (str.includes(target)) finalArray.push(str);
  })
  return finalArray;
...
const findEvery = (arr, target) => {
  let boolStatus = true;
  arr.forEach(str => {
    if (!str.includes(target)) boolStatus = false;
  })
  return boolStatus;
};
...
const unenrollBrook = (arr) => {
  let finalArray = [];
  arr.forEach(array => {
    let beforeFinalArray = [];
    array.forEach(item => {
      if (!item.includes('Brook')) beforeFinalArray.push(item);
    })
    finalArray.push(beforeFinalArray);
  })
  return finalArray;
};
